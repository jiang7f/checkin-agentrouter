...
GitHub OAuth 成功后会优先复用浏览器已返回的真实余额。如果浏览器只返回了零值占位数据，脚本会复用本次 OAuth 得到的新 session cookie 独立查询最多 3 次。余额查询失败不会重新执行 OAuth，本次通知会显示“余额获取失败”，新 session 仍会保存给下次签到前查询使用。
